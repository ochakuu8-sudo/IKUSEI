...
## 2026-09-07 / v14.6 デフォルメした執務室の素材

ユーザーの指定により、細密で写実的な古紙・革から、輪郭と丸みを強調したゲーム用イラストへ制作方針を修正。採用した6枚は built-in ImageGen で制作し、合計約247KiBのWebPとして保存した。

| ファイル（`public/art/ui/stylized/` 内） | 解像度 | 用途 |
|---|---|---|
| `paper-noble-v3.webp` | 612×643 | 貴族の便箋。金、藤色の薔薇 |
| `paper-academy-v3.webp` | 612×643 | 学院の便箋。銀、青緑の薔薇 |
| `paper-commerce-v3.webp` | 612×643 | 商会・組合の便箋。銅、クリームの薔薇 |
| `wax-seal-v2.webp` | 160×160、alpha | 厚みのある蝋印。既存7人の紋を重ねる |
| `ledger-book-v2.webp` | 916×416、alpha | 金具と薔薇を描いた葡萄色の台帳 |
| `desk-leather-v2.webp` | 1712×662 | 手紙の下に敷く革面 |

便箋は実寸合成で条件欄との干渉を確認し、角の花・葉を小さくしたv3を採用。カードは全体画像、詳細は角と辺を分けた描画で使う。文字・数字・人物紋・状態紋・署名は別レイヤー。参照は `src/art.ts` の `manorMaterialStyle` に集約する。

生成プロンプトと採用原画の対応は `docs/manor-art-prompts.json`。生成原画は作業フォルダにも保存済み。台帳の透過指定結果に残ったチェック柄は、外周から連続する無彩色背景だけを除去して切り抜きを確認した。最初の写実的な候補や大きい角飾りの候補は本番配信に含めない。
