import {
  ...
  isOpen, hasStaminaFor, hasStockFor, payWithRelation, capDropOf,
...
export function performAction(before: GameState, action: Action, offers: SupportOffer[] = supportOffers): ActionOutcome {
  try {
    ...
    let title = '', narrative = '', days = 0, worked: PersonId | 'none' = 'none', publicWork = false;
    ...
    if (action.type === 'job') {
      ...
      if (!person || s.relations[action.person] >= 3 || s.money < NETWORK_COST || s.stamina < NETWORK_STAMINA) fail('訪問の条件が足りません');
      ...
      const key = offerKey(s, offer!.id);
      title = offer!.title;
      if (action.type === 'decline') {
        s.offerStates[key] = 'declined'; narrative = '今回は引き受けない。代償はなく、次章には再び検討できる。';
      } else {
        ...
        s.obligations.push({ id: key, offerId: offer!.id, acceptedDay: absoluteDay(s), due: absoluteDay(s) + offer!.term,
      ...
      if (action.type === 'pay') {
        ...
        if (o.status !== 'active' || o.extensions >= o.terms.extensionLimit) fail('この約束はこれ以上延長できません');
    ...
    const result: DayResult = {
      ...
      learned: s.known.filter(id => !before.known.includes(id)), delivered, relationUp,
    };
