...
import { personOf, materialOf, recipeOf, type PersonId, type GameState } from './game';
...
export function SupportPanel({ game, onAction, onBack, allocation = false, focusOffer, focusObligation, person }: {
  game: GameState; onAction: (action: Action) => void; onBack: () => void; allocation?: boolean;
  focusOffer?: string; focusObligation?: string; person?: PersonId;
}) {
  const offers = supportOffers.filter(o => focusOffer ? o.id === focusOffer : person ? o.kind === 'credit' && o.person === person : false);
  const [tab, setTab] = useState<'offers' | 'promises'>(allocation || focusObligation || !offers.length ? 'promises' : 'offers');
  const [page, setPage] = useState(() => focusObligation
    ...
    : Math.max(0, offers.findIndex(o => o.id === focusOffer)));
  ...
  return <section className="support-panel">
    <div className="topbar">
      <button className="btn ghost sm" onClick={onBack}>{allocation ? '返済の確認へ' : '戻る'}</button>
      <h2>約束帳</h2><span className="topbar-sub">未精算 {outstandingTotal(game)}G</span>
    </div>
    <div className="support-nav">
      <button className={`btn sm ${tab === 'offers' ? '' : 'outline'}`} disabled={allocation || !offers.length} onClick={() => { setTab('offers'); setPage(0); setConfirm(null); }}>{person ? '掛け仕入れ' : '依頼の条件'}</button>
    ...
    {terms ? <article className="support-sheet">
      <header><small>{personOf(terms.person).name} ／ {terms.kind === 'advance' ? terms.schedule ? '特別依頼' : '旧予約注文' : '掛け仕入れ'}</small><h3>{terms.title}</h3></header>
      <div className="support-columns">
        <div>
          ...
          <dl>
            ...
            <dt>{promise ? '履行期限' : '期限'}</dt><dd>{terms.schedule ? `${dateLabel(terms.schedule.delivery)} 当日のみ ／ 受付 ${dateLabel(terms.schedule.appears)}〜${dateLabel(terms.schedule.closes)}` : promise ? dateLabel(promise.due) : `受諾日から${terms.term}日後 ／ 提示は今章${terms.closes}日まで`}</dd>
          ...
          <p className="support-note">{terms.rewards?.map(rewardLabel).join(" ／ ")}</p>
          <p className="support-note">{terms.schedule ? "指定日以前は準備のみ。期限延長はできません。" : <>再交渉：1日で{terms.extensionDays}日延長、最大{terms.extensionLimit}回。</>}履行で解禁：{terms.unlocks.map(id => capabilityLabels[id] ?? id).join('・') || '実績を記録'}。</p>
        </div>
        <div>
          {terms.options.map(c => <div className="support-option" key={c.id}>
            ...
            {promise?.status === 'active' && terms.schedule && absoluteDay(game) < promise.due && <p>{(game.stock[c.recipe] ?? 0) >= c.count ? '準備済み・指定日を待つ' : '準備中'}</p>}
          ...
          {offer && <>
            ...
            {!terms.schedule && button('今章は辞退する', { type: 'decline', offer: offer.id }, true)}
          </>}
          {promise && <>
            ...
            {promise.status === 'active' && !allocation && <div className="support-management">
              {!terms.schedule && button('1日使って期限を延ばす', { type: 'renegotiate', id: promise.id }, true)}
