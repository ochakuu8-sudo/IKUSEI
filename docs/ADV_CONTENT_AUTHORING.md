...
1. タイトルから「はじめから」。現在の本編は検証用の仮台本を共用する。この開始は現在の進行を置き換える。途中から確認するときは「続きから」を使う。
...
上記は現行挙動であり、縮小して収まるだけでは読みやすさの合格にならない。今後のUI変更・台本差し替えには[UI品質管理基準](UI_QUALITY_STANDARD.md)を適用する。変更した本文・選択・結果の最長箇所を1280×720と844×390で確認し、選択文・必要値・現在値が読めることを記録する。小さい文字や文字列別のCSSで長文を押し込まず、折返し・改ページ・単一一覧のスクロールで全文への到達を保つ。

同基準の長文データは耐性確認用であり、本番の人物名や台本の文字数制限を確定するものではない。新しい状態や想定外の長さを使う場合も、意味を削らず表示を確認する。現在のUIが新基準に未達の箇所は[改善の実装指示](UI_QUALITY_IMPLEMENTATION.md)へ記録されている。
